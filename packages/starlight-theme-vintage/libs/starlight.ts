...
export function overrideComponents(
  ...
  for (const override of overrides) {
    if (starlightConfig.components?.[override]) {
      ...
      logger.warn(
        `To use \`starlight-theme-vintage\`, either remove this override or manually render the content from \`${fallback}\`.`,
      )
      continue
    }
    components[override] = `starlight-theme-vintage/overrides/${override}.astro`
  }
